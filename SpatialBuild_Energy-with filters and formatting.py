...
def show_edit_modal(record_id, from_missing_data=False):
    ...
    # Use a SIMPLIFIED edit form without the complex unified function
    # This avoids the key duplication issues
    display_simple_edit_form(record_id, record_data, from_missing_data)
...
def review_missing_data():
    ...
    # Check if we need to show an edit modal
    ...
    if current_editing_record_id and current_editing_from == "missing_data":
        # Show the edit modal at the TOP of the page
    ...
    # Categorize records by missing data - THIS PART WAS COMMENTED OUT
...
def display_missing_record(item, category):
    ...
    with st.expander(f"Record {record_id}: {criteria} → {energy_method} ({direction})", expanded=False):
        ...
        with col2:
            # Check if we're already in edit mode for this record
            is_editing = st.session_state.get(f"edit_missing_record_{record_id}", False)
            
            if not is_editing:
                # Edit button - Use a callback pattern instead of direct rerun
                ...
                if st.button("✏️ Edit", key=unique_key, use_container_width=True):
                    # Store the record data and set edit mode WITHOUT immediate rerun
...
def display_edit_form(record_id, record):
    ...
    # Pass from_missing_data=True to maintain admin context
    ...
    saved = display_unified_edit_form(record_id, record_data, is_pending=False, clear_edit_callback=clear_edit_mode)
...
def review_pending_data():
    ...
    # Generate a unique session ID to avoid key conflicts
    import random
    session_id = f"{int(time.time())}_{random.randint(1000, 9999)}"
    
    # Create a refresh button at the top with a unique key
    refresh_top_key = f"refresh_top_{session_id}"
    if st.button("🔄 Refresh List", key=refresh_top_key, use_container_width=True):
        st.rerun()
    ...
    for index, record in enumerate(pending_records):
        ...
        # Create a unique key for this record display
        record_key = f"record_{record_id}_{session_id}_{index}"
        
        with st.container(key=f"container_{record_key}"):
            ...
            with col_header2:
                ...
                if not edit_mode:
                    edit_key = f"edit_btn_{record_key}"
            ...
            if st.session_state.get(f"admin_pending_edit_{record_id}"):
                ...
                content_key = f"content_{record_key}"
                st.text_area("Content", value=paragraph, height=150, key=content_key, disabled=True)
                
                # Admin approval/rejection buttons (view mode)
                col1, col2, col3 = st.columns([1, 1, 2])
                
                with col1:
                    approve_key = f"approve_{record_key}"
                    if st.button(f"✅ Approve", key=approve_key, use_container_width=True, type="primary"):
                        # Use global connection to update
                        cursor_update = conn.cursor()
                        cursor_update.execute("UPDATE energy_data SET status = 'approved' WHERE id = ?", (record_id,))
                        conn.commit()
                ...
                with col2:
                    reject_key = f"reject_{record_key}"
                    if st.button(f"❌ Reject", key=reject_key, use_container_width=True):
                        # Use global connection to update
                        cursor_update = conn.cursor()
                        cursor_update.execute("UPDATE energy_data SET status = 'rejected' WHERE id = ?", (record_id,))
                        conn.commit()
                        
                        # Show rejection message
                        st.error(f"Record {record_id} rejected and marked as rejected.")
                ...
                with col3:
                    ...
                    emoji = status_badge_color.get(status, '⚪')
                    # Simple text display instead of info to avoid key issues
                    st.markdown(f"**{emoji} Current Status:** **{status}**")
    ...
    cursor_stats.execute("SELECT COUNT(*) FROM energy_data WHERE status = 'pending'")
    pending_count = cursor_stats.fetchone()[0]
    
    cursor_stats.execute("SELECT COUNT(*) FROM energy_data WHERE status = 'approved'")
    approved_count = cursor_stats.fetchone()[0]
    
    cursor_stats.execute("SELECT COUNT(*) FROM energy_data WHERE status = 'rejected'")
    rejected_count = cursor_stats.fetchone()[0]
    
    # Create unique expander for statistics - FIXED: Remove the 'key' parameter
    ...
    # Refresh button at the bottom with different key
    refresh_bottom_key = f"refresh_bottom_{session_id}"
    if st.button("🔄 Refresh Pending List", key=refresh_bottom_key, use_container_width=True):
        st.rerun()
