...
def admin_dashboard():
    ...
    #     st.warning("This will modify location data in the database. Make sure you have a backup!")
        ...
        if st.button("Preview Location Cleanup"):
            ...
            cleanup_locations(st.session_state.db)    
...
def render_climate_distribution(unique_studies):
    ...
    # Count climates by UNIQUE STUDY (use climate code as key)
    climate_counts = {}
    ...
    if climate_counts:
        # Pass show_code=True to display both code and description
...
def render_scale_distribution(unique_studies):
    ...
    st.subheader("Scale Distribution (by unique study)")
    
    from color_schemes import get_scale_color
...
def render_building_use_distribution(unique_studies):
    ...
    st.subheader("Building Use Distribution (by unique study)")
    
    from color_schemes import get_building_use_color
...
def render_approach_distribution(unique_studies):
    ...
    st.subheader("Approach Distribution (by unique study)")
    
    from color_schemes import get_approach_color
...
''']
    ...
    for item, count in sorted_items:
        ...
        if description and show_code:
            # Show both code and description
            left_text = f"{item} - {description}"
        elif description:
            left_text = description
        else:
        ...
        with col_bar:
            # Simple color bar without text inside
            bar_html = f'''
...
def render_enhanced_papers_tab():
    ...
    with view_tab2:
        render_statistics_tab(st.session_state.db)
